...
async function main(): Promise<void> {
  ...
  const { initEnv } = await import("../src/internal/lib/env.js");
  const env = await initEnv();
  const { Logger } = await import("../src/internal/lib/logger.js");
  const { Database } = await import("../src/internal/lib/db/database.js");
