...
import { mockAnalytics, mockLogger, sampleMessage, flushQueue } from "../test/support/mocks.js";
...
function buildUseCase(overrides: {
  ...
  const blockOnboarding = {
    ...
    executeBlockWithSession: vi.fn(async () => Boolean(overrides.client))
  };

  const moderationReply = new ModerationReplyService(notifications as never, logger as never);
  const actionQueue = new ActionQueueService(logger as never, 0);
  ...
  const skipEvaluator = new ModerationSkipEvaluator(new InboundMessageDedupe(), analytics as never, logger as never);
  const priorBlockSkip = new PriorBlockSkipHandler(actions as never, analytics as never, logger as never);
...
describe("ProcessIncomingMessageUseCase", () => {
  ...
  it("sends text first then media as a follow-up when a variant has media", async () => {
    ...
    expect(notifications.sendBusinessMediaReply.mock.calls[0]?.[0]).toMatchObject({
      businessConnectionId: "bc-1",
      chatId: "chat-1",
      mediaPath: expect.stringContaining("variant-007.mp4")
    });
