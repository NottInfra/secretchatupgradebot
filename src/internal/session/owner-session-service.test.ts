...
function buildService(overrides: {
  ...
  restartOnboarding?: ReturnType<typeof vi.fn>;
  ...
  const provider = {
    ...
    startOnboarding:
      ...
      vi.fn(async () => {
        throw new Error("startOnboarding not mocked");
      }),
    restartOnboarding:
      overrides.restartOnboarding ??
      vi.fn(async () => {
        throw new Error("restartOnboarding not mocked");
      }),
...
describe("OwnerSessionService", () => {
  ...
  it("onboards when lookup returns empty shell and phone is provided", async () => {
    ...
    expect(client).toBe(tdlibClient);
    expect(provider.startOnboarding).toHaveBeenCalledWith(
  ...
  it("re-onboards when tdlib login fails but phone is provided", async () => {
    ...
    const restartOnboarding = vi.fn(async () => ({
      step: "complete",
      accountId: "acc-1"
    }));
    const { ownerSessions, logger, provider } = buildService({ restartOnboarding });
    const client = await ownerSessions.getTdlibForOwner("12345", "+447561231794");

    expect(client).toBe(tdlibClient);
    expect(provider.restartOnboarding).toHaveBeenCalledWith(
  ...
  it("rejects invalid onboarding responses", async () => {
    const { ownerSessions, logger } = buildService({
      ...
      startOnboarding: vi.fn(async () => "not-a-step" as never)
    });
    const client = await ownerSessions.getTdlibForOwner("12345", "+447700900123");
    expect(client).toBeUndefined();
